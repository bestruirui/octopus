...
type RelayLog struct {
	...
	RequestContent         string           `json:"request_content"`                          // 请求内容（转发给上游的内部请求）
	RequestFormat          string           `json:"request_format"`                           // 上游渠道 API 格式（openai/chat_completions 等）
	OriginalRequestType    string           `json:"original_request_type"`                    // 原始请求类型（chat/embedding/image 等）
	OriginalRequestFormat  string           `json:"original_request_format"`                  // 原始请求 API 格式（openai/chat_completions 等）
	OriginalRequestContent string           `json:"original_request_content"`                 // 原始请求内容（客户端发来的请求体）
