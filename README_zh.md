...
路由提供了一个智能路由层，**位于外部 API 调用与分组之间**。一个路由将对外模型名称映射到**主分组**（兜底），可选配置**分派分组**（LLM 智能分类）和多个**工作分组**（关键词路由）。
...
| 优先级 | 路径 | 说明 |
|--------|------|------|
| ① | **关键词匹配** | 检查用户消息是否包含工作分组的关键词/描述 → 路由到匹配的工作分组 |
| ② | **分派 LLM** | 使用分派分组的 LLM 对请求分类 → 路由到分类得到的工作分组 |
| ③ | **主分组兜底** | 以上均失败或返回无效分组时，回退到主分组 |

### 路由配置

一个路由包含：
- **主分组**（必选）：分派失效时的兜底分组
- **分派分组**（可选）：包含小模型/快模型的分组，用于请求分类
- **工作分组**（多个）：带有 `描述` + `关键词` 的分组，用于关键词匹配
...
└── 工作分组:
    ...
    ├── document (关键词: "document", "文档", "readme", "spec"...)
    └── tool-use (关键词: "tool", "工具", "function", "api"...)
```

### 请求流程示例

```
客户端: POST /v1/chat/completions { model: "think", messages: [...] }
                │
                ▼
路由 "think" 命中
                │
                ├─► 消息含 "分析" 关键词? ──是──► 分组 "analysis" ──► 渠道 A
                │      │
                │      否
                │      ▼
                ├─► 分派 LLM (lite 分组) 分类: "coding" ──是──► 分组 "coding" ──► 渠道 B
                │      │
                │      否 / 无效
                │      ▼
                └─► 回退到主分组 "main" ──► 渠道 C
...
- **智能路由**: 不同类型请求自动流向专业分组
- **成本优化**: 简单请求用廉价/快速模型；复杂请求用强模型
- **专业化分工**: 编码任务→编码优化模型；分析任务→推理模型
- **客户端零改动**: 客户端只需使用路由名作为 model；路由对客户端透明
